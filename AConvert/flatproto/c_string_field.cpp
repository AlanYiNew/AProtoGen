...
GenerateStringTlog(Printer* printer) {
  // 当前的tlog string接口和simple string接口对于 string/bytes 没有区别
...
GenerateCompareCAndCpp(Printer* printer) {
    ...
    printer->Print(vars_,
      ...
      "&$param_c_var$->$c_name$[0], $param_cpp_var$->$cpp_name$().size())) {\n"
...
GenerateAssignCToCpp(Printer* printer) {
    ...
    printer->Print(vars_, "$param_cpp_var$.set_$cpp_name$(std::string($param_c_var$.$c_name$");
...
GenerateAssignCppToC(Printer* printer) {
    ...
    if (IsBytes()) {
        printer->Print(vars_, 
                "int32_t $cpp_name$_len = (uint64_t)$param_cpp_var$.$cpp_name$().size() > sizeof($param_c_var$.$c_name$) ? "
                "sizeof($param_c_var$.$c_name$) : $param_cpp_var$.$cpp_name$().size();\n"
        ...
        printer->Print(vars_,
                "int32_t $cpp_name$_len = (uint64_t)$param_cpp_var$.$cpp_name$().size() >= sizeof($param_c_var$.$c_name$) ? "
                "sizeof($param_c_var$.$c_name$) - 1 : $param_cpp_var$.$cpp_name$().size();\n"
                "strncpy(&$param_c_var$.$c_name$[0], $param_cpp_var$.$cpp_name$().c_str(), $cpp_name$_len);\n"
                "$param_c_var$.$c_name$[$cpp_name$_len] = 0;\n");
    }
    return true;
}
...
GenerateCompareCAndCpp(Printer* printer) {
   ...
   printer->Print(vars_,
      "if (0 != memcmp($param_cpp_var$->$cpp_name$().c_str(),"
      "&$param_c_var$->$oneof_var$.$c_name$[0], $param_cpp_var$->$cpp_name$().size())) {\n"
      "   std::cout << \"$parent_type$::$c_name$ :\"\n"
      "             <<  \" element not equal\"\n"
      "             << std::endl;\n"
      "   return false;\n"
...
GenerateAssignCToCpp(Printer* printer) {
    printer->Print(vars_,
            "$param_cpp_var$.set_$cpp_name$(std::string($param_c_var$.$oneof_var$.$c_name$));\n");
    return true;
}
...
GenerateAssignCppToC(Printer* printer) {
    // 暂时这么处理后期将打印的字符串模板化
    printer->Print(vars_,
                "int32_t $cpp_name$_len = (uint64_t)$param_cpp_var$.$cpp_name$().size() >= sizeof($param_c_var$.$oneof_var$.$c_name$) ? "
                "sizeof($param_c_var$.$oneof_var$.$c_name$) - 1 : $param_cpp_var$.$cpp_name$().size();\n"
                "strncpy(&$param_c_var$.$oneof_var$.$c_name$[0], $param_cpp_var$.$cpp_name$().c_str(), $cpp_name$_len);\n"
                "$param_c_var$.$oneof_var$.$c_name$[$cpp_name$_len] = 0;\n");
    return true;
}
...
GenerateCompareCAndCpp(Printer* printer) {
    ...
    printer->Print(vars_, 
      ...
      "    $param_c_var$->$c_name$[i], $param_cpp_var$->$cpp_name$(i).size())) {\n"
...
GenerateAssignCToCpp(Printer* printer) {
    ...
    printer->Indent();
    printer->Print(vars_, 
            "$param_cpp_var$.add_$cpp_name$($param_c_var$.$c_name$[i]);\n");
    printer->Outdent();
    printer->Print("}\n");
    return true;
}
...
GenerateAssignCppToC(Printer* printer) {
    ...
    if (IsBytes()) {
        printer->Print(vars_, 
                "int32_t len = (uint64_t)$param_cpp_var$.$cpp_name$(i).size() >= sizeof($param_c_var$.$c_name$[i]) ?"
        ...
        printer->Print(vars_, 
                "int32_t len = (uint64_t)$param_cpp_var$.$cpp_name$(i).size() >= sizeof($param_c_var$.$c_name$[i]) ?"
                " sizeof($param_c_var$.$c_name$[i]) - 1 : $param_cpp_var$.$cpp_name$(i).size();\n");
        printer->Print(vars_, 
                "strncpy(&$param_c_var$.$c_name$[i][0], $param_cpp_var$.$cpp_name$(i).c_str(), len);\n"
                "$param_c_var$.$c_name$[i][len] = 0;\n");
    }
