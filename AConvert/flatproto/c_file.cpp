...
GenerateEncodeHeaderHead(Printer* printer) {
    ...
    printer->Print("#include <google/protobuf/wire_format_lite_inl.h>\n");
...
GetEncodeSpeedTestFun() {
    return "bool run_" + GetBaseProtoName(descriptor_->name()) +
        "_encode_speed_test(std::map<std::string, double>& c_map, std::map<std::string, double>& cpp_map)";
}

string CFileGenerator::
GetDecodeSpeedTestFun() {
    return "bool run_" + GetBaseProtoName(descriptor_->name()) +
        "_decode_speed_test(std::map<std::string, double>& c_map, std::map<std::string, double>& cpp_map)";
}
