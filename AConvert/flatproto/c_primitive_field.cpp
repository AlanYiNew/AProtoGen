...
GenerateAssignCToCpp(Printer* printer) {
    printer->Print(vars_, "$param_cpp_var$.set_$cpp_name$($param_c_var$.$c_name$);\n");
    return true;
}
    
bool CPrimitiveFieldGenerator::
GenerateAssignCppToC(Printer* printer) {
    printer->Print(vars_, "$param_c_var$.$c_name$ = $param_cpp_var$.$cpp_name$();\n");
   return true; 
}
...
GenerateCompareCAndCpp(Printer* printer) {
    printer->Print(vars_, 
            "if (fabs($param_cpp_var$->$cpp_name$() -"
            " $param_c_var$->$oneof_var$.$c_name$) > 0.000001) {\n"
            "  std::cout << \"$parent_type$::$c_name$ not equal [c:\"\n"
...
bool COneofPrimitiveFieldGenerator::
GenerateAssignCToCpp(Printer* printer) {
    printer->Print(vars_, "$param_cpp_var$.set_$cpp_name$($param_c_var$.$oneof_var$.$c_name$);\n");
    return true;
} 
  
bool COneofPrimitiveFieldGenerator::
GenerateAssignCppToC(Printer* printer) {
    printer->Print(vars_, "$param_c_var$.$oneof_var$.$c_name$ = $param_cpp_var$.$cpp_name$();\n");
    return true;
}
...
GenerateAssignCToCpp(Printer* printer) {
    ...
    printer->Indent();
    printer->Print(vars_, "$param_cpp_var$.add_$cpp_name$($param_c_var$.$c_name$[i]);\n");
    printer->Outdent();
    printer->Print(vars_, "}\n");
    return true;
}
