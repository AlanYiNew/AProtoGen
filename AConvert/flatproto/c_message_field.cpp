...
GenerateDecode(Printer* printer) {
    printer->Print(vars_,
            "$type$* temp = &$param_var$->$access_func$();\n");
...
GenerateAutoFillC(Printer* printer, bool full_fill) {
  ...
  printer->Print(vars_, 
          "if (random()%2) {\n"
          "  $fill_fun$(&$param_c_var$->$access_func$());\n"
          "}\n");
  return true;
}
...
GenerateCompareCAndCpp(Printer* printer) {
    ...
    printer->Print(vars_, 
      ...
      "if (!$compare_fun$(&$param_c_var$->$access_func$(),"
...
GenerateAssignCppToC(Printer* printer) {
    printer->Print(vars_,
            "if (!$cpp_2_c_fun$($param_cpp_var$.$cpp_name$(), $param_c_var$.$access_func$())) return false;\n");
    return true;
}
...
COneofMessageFieldGenerator(const FieldDescriptor* descriptor) 
  ...
  vars_["non_null_ptr_to_name"] = StrCat(vars_["param_var"],"->", camel, ".", descriptor_->name());
...
GenerateAutoFillC(Printer* printer, bool full_fill) {
  vars_["fill_fun"] = GetFillOrFillFullCFuncName(full_fill);
  printer->Print(vars_, 
        "$fill_fun$(&$param_c_var$->$oneof_var$.$c_name$);\n"
...
GenerateCompareCAndCpp(Printer* printer) {
    printer->Print(vars_, 
      "if (!$compare_fun$(&$param_c_var$->$oneof_var$.$c_name$,\n"
      "  $param_cpp_var$->mutable_$cpp_name$())) return false;\n");
    return true;
}
...
GenerateAssignCToCpp(Printer* printer) {
    printer->Print(vars_, 
            "if (!$c_2_cpp_fun$($param_c_var$.$oneof_var$.$access_func$, *$param_cpp_var$.mutable_$cpp_name$())) return false;\n");
    return true;
}
...
GenerateAssignCppToC(Printer* printer) {
    printer->Print(vars_,
            "if (!$cpp_2_c_fun$($param_cpp_var$.$cpp_name$(), $param_c_var$.$oneof_var$.$access_func$)) return false;\n");
    return true;
}
...
GenerateAssignCppToC(Printer* printer) {
    ...
    printer->Indent();
    printer->Print(vars_,
            "if (!$cpp_2_c_fun$($param_cpp_var$.$cpp_name$(i), $param_c_var$.$c_name$[i])) return false;\n");
    if (!GetFillFullExtOption(descriptor_)) {
