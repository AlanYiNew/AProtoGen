...
void SetCommVar(const FieldDescriptor* descriptor, 
    ...
    (*vars)["field_name"] = descriptor->name();
    ...
    if (oneof_descriptor) {
        ...
        (*vars)["non_null_ptr_to_name"] = 
            StrCat((*vars)["param_var"],"->", GetUnionWrapperName(descriptor), ".", descriptor->name());
        ...
        if (IsMessage(descriptor) && !IsRepeated(descriptor)) {
            (*vars)["non_null_ptr_to_name"] = 
                StrCat((*vars)["param_var"],"->", GetMessageFieldAccessFuncName(descriptor) + "()");
        } else {
            (*vars)["non_null_ptr_to_name"] = 
                StrCat((*vars)["param_var"], "->", descriptor->name());
        }
        (*vars)["non_null_cached_size_ptr_to_name"] = 
            StrCat((*vars)["param_cached_size"],"->", descriptor->name());
    }
...
void GenerateOneofStringSizeCompare(
    ...
    printer->Print((*vars),
            "if ($param_cpp_var$->$cpp_name$().size() != \n"
            "  strlen($param_c_var$->$oneof_var$.$c_name$)) {\n"
            "  std::cout << \" $parent_type$::$c_name$ size not eq:[c:\"\n"
            "       << strlen($param_c_var$->$oneof_var$.$c_name$)\n"
            "       << \", cpp:\"\n"
            "       << $param_cpp_var$->$cpp_name$().size()\n"
